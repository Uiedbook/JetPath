export class schema {
    ...
    async validateData(data) {
        ...
        for (const [prop, value] of Object.entries(this.schema)) {
            const { err, type, maxLength, minLength, mustInclude, mustNotBeLesserThan, mustNotBeGreaterThan, nullable, RegExp, validator, } = value;
            ...
            if (typeof data[prop] !== "undefined") {
                ...
                if (mustNotBeGreaterThan && Number(data[prop]) > mustNotBeGreaterThan) {
                    ...
                    throw new Error(` ${prop} must not be greater than ${mustNotBeGreaterThan}`);
                }
                ...
                if (mustNotBeLesserThan && Number(data[prop]) < mustNotBeLesserThan) {
                    ...
                    throw new Error(`Given ${prop} must not be lesser than ${mustNotBeLesserThan}`);
                }
                ...
                if (mustInclude &&
                    ...
                    throw new Error(` ${prop} does not includes ${[[...mustInclude].join(" , ")]}`);
                }
            }
            else {
                // additional prop
